...
class Spectral_Binary_Activation(nn.Module):
    ...
    def forward(self, x):
        ...
        out = (
            binary_activation_no_grad.detach()
            - tanh_activation.detach()
            + tanh_activation
        )
        return out
...
class HardBinaryConv(nn.Conv2d):
    def __init__(
        self, in_chn, out_chn, kernel_size=3, stride=1, padding=1, groups=1, bias=True
    ):
...
class BinaryConv2d_Down(nn.Module):
    ...
    def __init__(
        ...
        self.biconv_1 = BinaryConv2d(
            in_channels, in_channels, kernel_size, stride, padding, bias, groups
        )
        self.biconv_2 = BinaryConv2d(
            in_channels, in_channels, kernel_size, stride, padding, bias, groups
        )
...
class BinaryConv2d_Up(nn.Module):
    ...
    def __init__(
        ...
        self.biconv_1 = BinaryConv2d(
            out_channels, out_channels, kernel_size, stride, padding, bias, groups
        )
        self.biconv_2 = BinaryConv2d(
            out_channels, out_channels, kernel_size, stride, padding, bias, groups
        )
...
class FeedForward(nn.Module):
    def __init__(self, dim, mult=2):
        ...
        self.net = nn.Sequential(
            ...
            BinaryConv2d_Fusion_Increase(
                dim * mult, dim * mult * mult, 1, 1, bias=False
            ),
            RPReLU(dim * mult * mult),
            BinaryConv2d(
                dim * mult * mult, dim * mult * mult, 3, 1, 1, bias=False, groups=dim
            ),
            RPReLU(dim * mult * mult),
            BinaryConv2d_Fusion_Decrease(
                dim * mult * mult, dim * mult, 1, 1, bias=False
            ),
...
class BiSRNet_body(nn.Module):
    def __init__(self, in_dim=28, out_dim=28, dim=28, stage=2, num_blocks=[2, 4, 4]):
        ...
        # Input projection
        self.embedding = BinaryConv2d(
            in_dim, self.dim, 3, 1, 1, bias=False
        )  # 1-bit -> 32-bit
            ...
            self.encoder_layers.append(
                nn.ModuleList(
                    [
                        ...
                        BinaryConv2d_Down(
                            dim_stage, dim_stage * 2, 3, 1, 1, bias=False
                        ),
                    ]
                )
            )
            ...
            self.decoder_layers.append(
                nn.ModuleList(
                    [
                        ...
                        BinaryConv2d_Fusion_Decrease(
                            dim_stage, dim_stage // 2, 1, 1, bias=False
                        ),
                        BiSRNet_Block(
                            dim=dim_stage // 2,
        ...
        # Output projection
        self.mapping = BinaryConv2d(
            self.dim, out_dim, 3, 1, 1, bias=False
        )  # 1-bit -> 32-bit
...
class BiSRNet(nn.Module):
    ...
    def __init__(
        self, in_channels=28, out_channels=28, n_feat=28, stage=3, num_blocks=[1, 1, 1]
    ):
        ...
        modules_body = [
            BiSRNet_body(dim=n_feat, stage=2, num_blocks=num_blocks)
