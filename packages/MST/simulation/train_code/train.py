...
train_set = LoadTest(opt.test_path)
...
# model = model_generator("dauhst_3stg", "./MST/simulation/test_code/model_zoo/dauhst_3stg/dauhst_3stg.pth").cuda()
...
def train(epoch, logger):
    ...
    with tqdm(total=batch_num, desc=f"Epoch {epoch}", unit="batch") as pbar:
        for i in range(batch_num):
            ...
            # Update the progress bar and show per-batch loss
    ...
    average_loss = epoch_loss / batch_num
    logger.info("===> Epoch {} Complete: Avg. Loss: {:.6f} time: {:.2f}".format(epoch, average_loss, (end - begin)))
