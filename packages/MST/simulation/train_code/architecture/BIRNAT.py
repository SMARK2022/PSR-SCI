...
class self_attention(nn.Module):
    ...
    def forward(self, x):
        ...
        n = torch.matmul(
            ft.permute([0, 2, 1]), g.reshape([batch_size, self.ch // 8, -1])
        )
...
class forward_rnn(nn.Module):
    ...
    def __init__(self):
        ...
        self.conv_x1 = nn.Sequential(
            ...
            nn.LeakyReLU(inplace=True),
            nn.ConvTranspose2d(
                20, 10, kernel_size=3, stride=2, padding=1, output_padding=1
            ),
        )
...
class backrnn(nn.Module):
    ...
    def forward(
        ...
        for i in range(cs_rate - 1):
            ...
            for ii in range(i + 1):
                d1 = d1 + torch.mul(
                    mask3d_batch[:, cs_rate - 1 - ii, :, :],
                    out[:, cs_rate - 1 - ii, :, :].clone(),
                )
            for ii in range(i + 2, cs_rate):
                d2 = d2 + torch.mul(
                    mask3d_batch[:, cs_rate - 1 - ii, :, :],
                    xt8[:, cs_rate - 1 - ii, :, :].clone(),
                )
            x12 = self.conv_x2(torch.unsqueeze(meas - d1 - d2, 1))
...
class BIRNAT(nn.Module):
    ...
    def gen_meas_torch(self, meas, shift_mask):
        ...
        temp = torch.mul(
            torch.unsqueeze(nor_meas, dim=1).expand(
                [batch_size, 28, H, shift_mask.shape[3]]
            ),
            shift_mask,
        )
    ...
    def forward(self, meas, shift_mask=None):
        ...
        model_out1, h1 = self.rnn1(
            xt1, meas, nor_meas, PhiTy, shift_mask, h0, self.cs_rate
        )
        model_out2 = self.rnn2(
            model_out1, meas, nor_meas, PhiTy, shift_mask, h1, self.cs_rate
        )
