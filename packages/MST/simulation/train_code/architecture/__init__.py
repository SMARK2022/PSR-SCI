...
def model_generator(method, pretrained_model_path=None):
    if method == "mst_s":
        ...
        model = MST(dim=28, stage=5, num_blocks=[8, 15, 13, 11, 9]).cuda()
        # model = MST(dim=28, stage=5, num_blocks=[12, 23, 21, 17, 15]).cuda()
        # model = MST(dim=28, stage=6, num_blocks=[16, 31, 29, 27, 25, 21]).cuda()
    elif method == "gap_net":
        model = GAP_net().cuda()
    ...
    print(f"Total number of parameters in the model ({method}): {num_params}")
