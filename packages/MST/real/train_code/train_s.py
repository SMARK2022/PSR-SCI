...
if __name__ == "__main__":
    ...
    for epoch in range(0, opt.max_epoch):
        ...
        for i, (input, label, Mask, Phi, Phi_s) in enumerate(loader_train):
            ...
            if i % (1000) == 0:
                print("%4d %4d / %4d loss = %.10f time = %s" % (epoch + 1, i, len(Dataset) // opt.batch_size, epoch_loss / ((i + 1) * opt.batch_size), datetime.datetime.now()))

        elapsed_time = time.time() - start_time

        logger.info("===> Epoch {} Complete: Avg. Loss: {:.6f} time: {:.2f} s".format(epoch + 1, epoch_loss / len(Dataset), elapsed_time))
