...
class HS_MSA(nn.Module):
    def __init__(self, dim, window_size=(8, 8), dim_head=28, heads=8, only_local_branch=False):
        super().__init__()
    ...
    def forward(self, x):
        ...
        if self.only_local_branch:
            ...
            # local branch
            q1, k1, v1 = map(lambda t: rearrange(t, "b (h b0) (w b1) c -> b (h w) (b0 b1) c", b0=w_size[0], b1=w_size[1]), (q1, k1, v1))
            ...
            # non-local branch
            q2, k2, v2 = map(lambda t: rearrange(t, "b (h b0) (w b1) c -> b (h w) (b0 b1) c", b0=w_size[0], b1=w_size[1]), (q2, k2, v2))
...
class HSAB(nn.Module):
    def __init__(
        ...
        for _ in range(num_blocks):
            self.blocks.append(nn.ModuleList([PreNorm(dim, HS_MSA(dim=dim, window_size=window_size, dim_head=dim_head, heads=heads, only_local_branch=(heads == 1))), PreNorm(dim, FeedForward(dim=dim))]))
            ...
            self.decoder_layers.append(
                nn.ModuleList(
                    [
                        ...
                        HSAB(dim=dim_scale // 2, num_blocks=num_blocks[self.scales - 2 - i], dim_head=dim, heads=(dim_scale // 2) // dim),
                    ]
                )
            )
            dim_scale //= 2
...
class HyPaNet(nn.Module):
    def __init__(self, in_nc=29, out_nc=8, channel=64):
        ...
        self.mlp = nn.Sequential(nn.Conv2d(channel, channel, 1, padding=0, bias=True), nn.ReLU(inplace=True), nn.Conv2d(channel, channel, 1, padding=0, bias=True), nn.ReLU(inplace=True), nn.Conv2d(channel, out_nc, 1, padding=0, bias=True), nn.Softplus())
