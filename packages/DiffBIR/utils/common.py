...
def wavelet_blur_msi(image: Tensor, radius: int):
    """
    Apply wavelet blur to the input tensor.
    """
    # input shape: (1, 28, H, W)
    # convolution kernel
    kernel_vals = [
        [0.0625, 0.125, 0.0625],
        [0.125, 0.25, 0.125],
    ...
    image = F.pad(image, (radius, radius, radius, radius), mode="replicate")
    # apply convolution
    output = F.conv2d(image, kernel, groups=28, dilation=radius)
    return output


def wavelet_decomposition_msi(image: Tensor, levels=3):
    """
    ...
    for i in range(levels):
        radius = 2**i
        low_freq = wavelet_blur_msi(image, radius)
