...
class MSI_Pipeline:
    ...
    def run_stage2(
        ...
        with torch.no_grad():
            bs, _, ori_h, ori_w = clean.shape
            # pad: ensure that height & width are multiples of 64
            bias = clean.mean(dim=(1, 2, 3))[:, None, None, None]
            if not self.cond_fn is None:
                self.cond_fn.load_bias(bias)
                self.cond_fn.load_rgb_target(clean)

            clean = clean - bias + 0.5
            ...
            if better_start:
                ...
                # reverse sampling, which can prevent our model from generating noise in
                # image background.
                _, low_freq = wavelet_decomposition(pad_clean, 2)
                if not tiled:
                    x_0 = self.cldm.vae_encode(low_freq)
                else:
                    x_0 = self.cldm.vae_encode_tiled(low_freq, tile_size, tile_stride)
            ...
            sampler = SpacedSampler(self.diffusion.betas)
            z = sampler.sample(
                model=self.cldm, device=self.device, steps=steps, batch_size=bs, x_size=(4, h // 8, w // 8),upscale=upscale,
                cond=cond, uncond=None, cfg_scale=cfg_scale, x_T=x_T, progress=True,
                progress_leave=True, cond_fn=self.cond_fn, tiled=tiled, tile_size=tile_size, tile_stride=tile_stride
            )
            if not tiled:
                x = self.cldm.vae_decode(z) / 2 + bias
            else:
                x = self.cldm.vae_decode_tiled(z, tile_size // 8, tile_stride // 8)
            ### postprocess
            self.cldm.control_scales = old_control_scales
